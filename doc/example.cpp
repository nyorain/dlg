#include <dlg/dlg.hpp> // dlg macros and basic source stuff
#include <dlg/output.hpp> // needed for custom output handler set later
using namespace dlg::literals; // for "_source, "_module etc
...
int main()
{
	...
	// alternatively use the dlg free function like dlg::scope
	dlg_info("main"_scope, "Use dlg sources to show where a log call came from");
	dlg_info("main"_scope, "We are in the main function");
	dlg_info("main"_scope, "That can also be done using a scoped source guard");

	{
		// using a dlg_source guard
		// note this time we are using the _src string literal which will
		// parse the separated source blocks
		// This results in:
		// - project: "dlg"
		// - module: "example"
		// - scope: "main_sub"
		dlg_source("dlg::example::main_sub");

		dlg_info("Now we are using the source specified above");
		dlg_info("If this output is confusing for you, look at example.cpp");
	}
	...
	dlg::output_handler([](const dlg::Origin& origin, std::string_view msg){
		...
		// we could e.g. also filter out certain sources
		// src level 1 is "module"
		if(origin.source.src[1] == "filtered")
			return;

		// depending on source/type/level or even based on the messages content (probably
		...
		// of formatting the origin (source/type/expression/level/file/line) and color
	...
	dlg_critical("filtered"_module, "I am {}: feelsbadman", "useless"); // not printed, filtered
	...
	dlg_trace("filtered"_module, "I am printed again, yeay"); // printed to cout

	dlg_info("If you don't like the colors here, don't despair for they may be changed!");
	dlg_info("Also make sure to check out the assert_log file in your cwd, there should be one\n");
